...
work_dir = 'results//louvain_algorithm//'+matching_setting+'//'+measures_setting
#%%
...
G = toSingleGraph(G_mult) # convert to single graph
...
#%%
# Evaluate different resolution values:
evaluation_results = {}

setting_count = 0
resolution = resolution_min
while (resolution <= resolution_max):
     
    # log file initialization:
    f_out = open(work_dir+'//log//'+name+'_'+str(resolution)+'.txt','w')
    ...
    for h in range(k): # repeat process k times
        ...
        for j in range(k):
            ...
            # Some statistics:
            groups_unique,groups_sizes = np.unique(groups.values(),return_counts=True)
...
for item in range(len(evaluation_results)):
    f_out.write(str(evaluation_results[item]['resolution'])+'\t'+str(round(evaluation_results[item]['AUC'],4))+'\t'+str(round(evaluation_results[item]['AUC_std'],4))+'\t'+str(round(evaluation_results[item]['AUPR'],4))+'\t'+str(round(evaluation_results[item]['AUPR_std'],4))+'\t'+str(round(evaluation_results[item]['time'],2))+'\n')
...
f_out.write(str(best_resolution)+'\t'+str(round(evaluation_results[index_best]['AUC'],4))+'\t'+str(round(evaluation_results[index_best]['AUC_std'],4))+'\t'+str(round(evaluation_results[index_best]['AUPR'],4))+'\t'+str(round(evaluation_results[index_best]['AUPR_std'],4))+'\n')
